...
if __name__ == '__main__':
    ...
    if opts.preload_path is None:
        ...
        if opts.per:
            agent.buffer = ExperienceReplay.clone_buffer_per(new_buffer=agent.buffer, maxsize=int(opts.buffer_size), alpha=opts.per_alpha, beta=opts.per_beta, beta_end=opts.per_beta_end)
        else:
            agent.buffer = ExperienceReplay.clone_buffer(agent.buffer, opts.buffer_size)
